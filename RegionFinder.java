 ...
 * @author Godwin Kangor, Dartmouth CS10
 */
public class RegionFinder {
    private static final int maxColorDiff = 26;				// how similar a pixel color must be to the target color, to belong to a region
    ...
    public void findRegions(Color targetColor) {
        ...
        // Loop through all pixels(Nested loop)
        for (int y = 0; y < image.getHeight(); y ++) {
            for (int x = 0; x < image.getWidth(); x++) {
                ...
                if (visited.getRGB(x,y) == 0 && colorMatch(firstColor, targetColor) ) {
                    ...
                    Stack<Point> toVisit = new Stack<>();// stack (toVisit) is created to store Point objects representing pixel coordinates in the image that need to be visited.
                    ...
                    while(!toVisit.empty()) {
                        ...
                        region.add(curr);
//                        System.out.println(curr.x - 1 < 0 );0//debugging
                    ...
                    // add it to the list of regions so it can be checked in largest region finding method
    ...
    protected static boolean colorMatch(Color c1, Color c2) {
        ...
        boolean isClose = false;// comparing the absolute chanel
